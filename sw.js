console.log("\n CONNECTION FROM WORKER: ", navigator.connection);
...
const cacheAssets = async () => {
  ...
  const cache = await caches.open("chivingtoninc-v1");
...
self.addEventListener("fetch", event => event.respondWith(cachedResource(event.request)));
